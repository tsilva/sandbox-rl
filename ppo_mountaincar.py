"""
PPO (Proximal Policy Optimization) implementation for MountainCar-v0.
Solves the environment when average reward >= -110 over 100 episodes.

Usage:
    python ppo_mountaincar.py          # Train and save policy
    python ppo_mountaincar.py --play   # Play back saved policy with rendering
"""
...
def shape_reward_vectorized(states, next_states, rewards, dones):
    """Shape rewards for vectorized environments."""
    positions = next_states[:, 0]
    velocities = next_states[:, 1]

    # Height reward (position ranges from -1.2 to 0.6)
    height_reward = (positions + 1.2) * 0.1

    # Velocity reward (momentum is key)
    velocity_reward = np.abs(velocities) * 0.5

    # Goal bonus
    goal_bonus = np.where(positions >= 0.5, 100.0, 0.0)
...
def train(session_path: str = None, reason: str = "Initial training attempt"):
    ...
    n_steps = 128  # Steps per env per rollout (total = n_envs * n_steps = 2048)
    batch_size = 64
    n_epochs = 10
    lr = 3e-4
    gamma = 0.99
    ...
    for iteration in range(max_iterations):
        ...
        for step in range(n_steps):
            ...
            # Shape rewards for training
            shaped_rewards = shape_reward_vectorized(states, next_states, rewards, dones)

            buffer.add(states, actions, shaped_rewards, dones.astype(np.float32), log_probs, values)
            episode_reward_buffer += rewards  # Track original rewards
